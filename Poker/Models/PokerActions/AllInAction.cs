namespace Poker.Models.PokerActions
{
    ...
    public class AllInAction : Action
    {
        ...
        public override void ApplyAction()
        {
            ...
            {
                var newCallAmount = Math.Min(
                    Pot.Instance.AmountRaisedTo - this.Player.RaiseAmount,
                    this.Player.ChipsSet.Amount);
                ((Human)this.Player).CallButton.Text = string.Format("{0} {1}", Actions.Call, newCallAmount);
            }

            this.Players.AddRange(this.Players.GetRange(0, this.PlayerIndex));
            this.Players.RemoveRange(0, this.PlayerIndex);
            this.PlayerIndex = -1;
            this.Players =
                this.Players.Where(player => (player.IsFolded == false) && player.ChipsSet.Amount > 0).ToList();
        }
    }
}