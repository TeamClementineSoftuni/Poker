...
namespace Poker.ArtificialIntelligence
{
    ...
    {
        public static void HP(Player player,  int n, int n1,ref int call, ref double Raise, ref bool raising, TextBox tbPod)
        {
            ...
            if (call > 0)
            {
                if (rnd == 1)
                {
                    ...
                    {
                        SomeMethods.Call(player,call,ref raising, tbPod);
                    }
                    else
                    {
            ...
            if (rnd == 3)
            {
                ...
                {
                    Raise = call * 2;
                    SomeMethods.Raised(player,  ref call,ref raising,tbPod, ref Raise);
                }
                else
                {
        ...
        public static void PH(Player player,   int n, int n1, int r, ref double rounds, ref int call, ref double Raise, ref bool raising, TextBox tbPot)
        {
            Random rand = new Random();
            ...
            if (rounds < 2)
            {
                ...
                if (call > 0)
                {
                    ...
                    if (!player.FoldedTurn)
                    {
                        ...
                        {
                            SomeMethods.Call(player, call, ref raising, tbPot);
                        }
                        ...
                        if (Raise <= (SomeMethods.RoundN(player.ChipsSet.Amount, n)) / 2)
                        {
                            if (Raise > 0)
                            {
                                ...
                                SomeMethods.Raised(player,   ref call, ref raising, tbPot, ref Raise);
                            }
                            else
                            {
                                Raise = call * 2;
        ...
        public static void Smooth(Player player,  int name, int n, int r, ref int call, ref double Raise,ref bool raising, TextBox tbPot)
        {
            Random rand = new Random();
            int rnd = rand.Next(1, 3);
            if (call <= 0)
